            ...
            <div className="col-12 col-md-5">
              <div className="d-flex align-items-center gap-3">
                ...
                <div>
                  ...
                  <p className="small text-muted mb-0">Production-grade prompts for Vibe Coding & AI tools</p>
                </div>
              </div>
            </div>
        ...
        <div className="row g-4">
          ...
          <div className="col-lg-3 sidebar-column">
            <div className="sidebar-sticky glass-panel p-3">
              <h5 className="mb-3 px-2 text-uppercase fs-6 tracking-wider text-muted">Categories</h5>
              <div className="nav flex-column">
                {categories.map(cat => (
                  ...
                  >
                    <i className={`bi ${selectedCategory === cat.name ? 'bi-folder2-open' : 'bi-folder2'} me-2`}></i>
                    <span className="flex-grow-1 text-truncate">{cat.name}</span>
                    <span className="badge rounded-pill bg-dark text-muted border border-secondary-subtle small px-2 py-1">{cat.count}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="col-lg-9">
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
              <div>
                <h2 className="h4 mb-0 font-outfit">
                  {selectedCategory === 'All' ? 'All Curated Prompts' : selectedCategory}
                </h2>
                <p className="text-muted small mb-0">
                  Showing {filteredPrompts.length} of {promptsData.length} production prompts
                </p>
              </div>
            </div>
            ...
            {filteredPrompts.length === 0 && (
              <div className="glass-panel empty-state my-5">
                ...
                <p className="text-muted">We couldn't find anything matching your search term. Try another keyword.</p>
                <button 
                  ...
                  <div className="col-12" key={prompt.id}>
                    <div className="prompt-card">
                      ...
                      <div className="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                        ...
                        >
                          <i className={`bi ${isCopied ? 'bi-check-circle-fill' : 'bi-clipboard'}`}></i>
                      ...
                      {prompt.placeholders && prompt.placeholders.length > 0 && (
                        <div className="bg-dark bg-opacity-25 rounded-3 p-3 mb-3 border border-secondary-subtle">
                          <span className="small text-muted fw-bold d-block mb-2 uppercase-tracking">
                            <i className="bi bi-sliders me-1"></i> Interactive Placeholders (Optional)
                          </span>
                          <div className="row g-2">
                            {prompt.placeholders.map(ph => (
                              <div className="col-12 col-sm-6" key={ph}>
                                <div className="input-group input-group-sm">
                                  <span className="input-group-text bg-dark border-secondary-subtle text-muted text-monospace" style={{fontSize: '0.75rem'}}>
                                    {ph}
                                  </span>
                                  <input
                                    type="text"
                                    ...
                                    value={placeholderInputs[`${prompt.id}-${ph}`] || ''}
                                    onChange={(e) => handlePlaceholderChange(prompt.id, ph, e.target.value)}
                                  />
                                </div>
                              </div>
                            ))}
                      ...
                      <div className="code-container">
                        <pre><code>{substitutedContent}</code></pre>
                      </div>
                    </div>
                  </div>
                );
      ...
      <footer className="mt-auto py-4 border-top bg-dark bg-opacity-50">
        <div className="container text-center">
          <p className="mb-1 footer-text">
            <strong>Prompt Playbook</strong> &copy; {new Date().getFullYear()}. Curated prompts for senior engineers.
          </p>
          <p className="mb-0 text-muted small">
            Designed for ChatGPT, Claude, Gemini, Cursor, Windsurf, Cline, Roo Code and other AI development tools.
          </p>
        </div>
      </footer>
    </div>
